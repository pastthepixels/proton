...
function init( scripts ) {
	...
	if ( ! scripts ) {
		...
		scripts = {
			...
			physijs: "https://rawcdn.githack.com/chandlerprall/Physijs/48a1b454c72a4a4baffce3f90ea46af5b2644bc2/dist/physi.js"
		};
...
// 				the user.
//
var Proton3DInterpreter = {

	// creating and modifing Proton3DScenes
	create3DScene( extras, scene ) {

		// this part requires an internet connection
		Physijs.scripts = Physijs.scripts || {}
		Physijs.scripts.worker = "https://rawcdn.githack.com/chandlerprall/Physijs/48a1b454c72a4a4baffce3f90ea46af5b2644bc2/dist/physijs-worker.js";
		...
		// physics
		this.objects = new Physijs.Scene( Physijs.scripts.worker );
		this.physicsFramerate = 1000 / 60;
		function renderPhysics() {
		
			setTimeout( function() {
				
				// Renders physics
				Proton3DInterpreter.objects.step.bind( Proton3DInterpreter.objects, Proton3DInterpreter.physicsFramerate / 1000, undefined, renderPhysics )()
			
			}, Proton3DInterpreter.physicsFramerate );
		
		}
		this.objects.step( Proton3DInterpreter.physicsFramerate / 1000, undefined, renderPhysics );
	...
	create3DObject( extras, object ) {
		...
		switch ( extras.type ) {
			...
			case "import":
				console.error( "'import' Proton3DObject type not supported. Instead, use Proton.import (because the variable 'type' is used to denote file type)" );
			...
			case "cube":
				...
				object.setWidth = ( value ) => {
					...
					object._width = value;
					Proton3DInterpreter.createMeshGeometry( object, object, object.name );
			...
			case "cylinder":
				...
				object.setRadiusTop = ( value ) => {
					...
					Proton3DInterpreter.createMeshGeometry( object );
	...
	Proton3DObject: {
		...
		setLinearVelocity( x = getMeshByName( P3DObject.name ).getLinearVelocity().x, y = getMeshByName( P3DObject.name ).getLinearVelocity().y, z = getMeshByName( P3DObject.name ).getLinearVelocity().z, P3DObject ) {
			...
			getMeshByName( P3DObject.name ).physics.linear_velocity.set( x, y, z );
		...
		setAngularVelocity( x = getMeshByName( P3DObject.name ).getAngularVelocity().x, y = getMeshByName( P3DObject.name ).getAngularVelocity().y, z = getMeshByName( P3DObject.name ).getAngularVelocity().z, P3DObject ) {
			...
			getMeshByName( P3DObject.name ).physics.angular_velocity.set( x, y, z );
		...
		setLinearFactor( x = 0, y = 0, z = 0, P3DObject ) {
			...
			getMeshByName( P3DObject.name ).physics.linear_factor.set( x, y, z );

		},
		setAngularFactor( x = 0, y = 0, z = 0, P3DObject ) {

			getMeshByName( P3DObject.name ).physics.angular_factor.set( x, y, z );
		...
		getLinearVelocity( P3DObject ) {
			...
			return getMeshByName( P3DObject.name ).physics.linear_velocity;

		},
		getAngularVelocity( P3DObject ) {

			return getMeshByName( P3DObject.name ).physics.angular_velocity;
	...
	importObject: function ( extras ) {
		...
		function loadObjectPhysics( child, i ) {
			...
			var physicalObject = new Physijs[ m ](
				c.geometry,
				c.material,
...
class PlayerPhysics {
	...
	constructor( player ) {
		...
		this.player = player;
		this.frame = 0;
		this.gravity = -.1 // Metres per second
		this.refreshRate = 16 // Milliseconds

		// Repeats updating
		setInterval( function() { physics.update() }, this.refreshRate );

	}

	// Updates physics simulation per frame
	update() {

		this.frame ++;
		this.player.setPosition( undefined, this.player.getPosition().y + ( this.gravity * ( this.refreshRate / 100 ) ), undefined );
